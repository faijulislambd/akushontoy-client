...
const MyToys = () => {
  ...
  useEffect(() => {
    let url =
      searchText !== ""
        ? `https://akushontoy-server.vercel.app/mytoys?uploaded_by=${user.email}&toy_name=${searchText}`
        : `https://akushontoy-server.vercel.app/mytoys?uploaded_by=${user.email}`;
    ...
    <Container className="py-5">
      ...
      {loadedToys.length > 0 ? (
        ...
        <h3 className="fs-1 text-center my-3">
          Sorry You Haven't Added Any Toy's Yet!
        </h3>
      )}
    </Container>
  );
